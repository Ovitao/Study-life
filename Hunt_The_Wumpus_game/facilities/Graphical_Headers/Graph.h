...
struct Color {
	...
	Color(Fl_Color fl) :c(fl), v(visible) { }
...
public:
	...
	Vector_ref() {}
	Vector_ref(initializer_list<T*>lst)
	{
		for (auto t: lst)
			push_back(t);
	}
...
template<class T>
class Vector_ref<T>::const_iterator :public vector<T*>::const_iterator
{
	//T* el;
	//int pos{ 0 };
//	Vector_ref<T>* base;
public:
	const_iterator(typename vector<T*>::const_iterator ci)
		:vector<T*>::const_iterator{ci}{}
	//using vector<T*>::const_iterator::const_iterator;
	/*
	const_iterator(Vector_ref<T>* v)
		:base{ v }, el{ base->size()? &(*base )[0]:0}
	{
	
		v->owned();
	}*/
	T& operator*()const {
		return *vector<T*>::const_iterator::operator*();
		//return *pos;
	}/*
...
template<class T> 
typename  Vector_ref<T>::const_iterator Vector_ref<T>::begin() const
{
	return v.begin();
}
template<class T>

typename  Vector_ref<T>::const_iterator Vector_ref<T>::end() const
{
	return v.end();
}
